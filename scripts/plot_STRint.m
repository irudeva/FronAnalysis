...
for issn = 1:nssn
    ...
    for isec = nsec:-1:1
        isec
        if isec == nsec ||  isec == 2
          p(isec) = plot(squeeze(yrs(1,:)), squeeze(STRint(isec,issn,:)),':');
        else
          p(isec) = plot(squeeze(yrs(1,:)), squeeze(STRint(isec,issn,:)),'-');
        end
        -isec+nsec+1
...
%        lgd.Location = 'bestoutside'
    ...
    for isec = 1:nsec
        ...
        r = corrcoef(STRint(1,issn,i1:end),STRint(isec,issn,i1:end));
        R_STRint(issn, isec) = r(1,2);
        stdSTRint(issn, isec) = std(STRint(isec,issn,i1:end));
        clear r
    end 
